...
const Carbos = () => {
    ...
    const saveItem = async (valor)=>{
        if(fakeId=== ''){
            await db.collection('carbo').doc().set(valor)
        }
        else{
            await db.collection('carbo').doc(fakeId).update(valor)
        }
        setFakeId('')
    }

    const deleteItem = async (valor)=>{
        await db.collection('carbo').doc(valor).delete()
    }

    const getItems = ()=>{
        db.collection('carbo').onSnapshot((querySnapshot)=>{
            const docs = []
    ...
    const getOne = async (id)=>{
       const doc =  await db.collection('carbo').doc(id).get()
        setItem({...doc.data()})
    }
    ...
    return (
      <div>
        <h1 className="text-center mt-5 mb-5">Alimentos ricos en carbohidratos</h1>
        {/* formulario */}
