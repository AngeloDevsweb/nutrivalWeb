...
const Grasas = () => {
    ...
    const saveItem = async (valor)=>{
        if(fakeId=== ''){
            await db.collection('grasa').doc().set(valor)
        }
        else{
            await db.collection('grasa').doc(fakeId).update(valor)
        }
        setFakeId('')
    }

    const deleteItem = async (valor)=>{
        await db.collection('grasa').doc(valor).delete()
    }

    const getItems = ()=>{
        db.collection('grasa').onSnapshot((querySnapshot)=>{
            const docs = []
    ...
    const getOne = async (id)=>{
       const doc =  await db.collection('grasa').doc(id).get()
        setItem({...doc.data()})
    }
    ...
    return (
      <div>
        <h1 className="text-center mt-5 mb-5">Alimentos ricos en grasas</h1>
        {/* formulario */}
