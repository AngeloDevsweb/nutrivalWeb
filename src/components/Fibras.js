...
const Fibras = () => {
    ...
    const saveItem = async (valor)=>{
        if(fakeId=== ''){
            await db.collection('fibra').doc().set(valor)
        }
        else{
            await db.collection('fibra').doc(fakeId).update(valor)
        }
        setFakeId('')
    }

    const deleteItem = async (valor)=>{
        await db.collection('fibra').doc(valor).delete()
    }

    const getItems = ()=>{
        db.collection('fibra').onSnapshot((querySnapshot)=>{
            const docs = []
    ...
    const getOne = async (id)=>{
       const doc =  await db.collection('fibra').doc(id).get()
        setItem({...doc.data()})
    }
    ...
    return (
      <div>
        <h1 className="text-center mt-5 mb-5">Alimentos ricos en fibras</h1>
        {/* formulario */}
